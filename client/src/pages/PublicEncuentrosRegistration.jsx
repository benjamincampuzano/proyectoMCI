...
const PublicEncuentrosRegistration = () => {
    ...
    useEffect(() => {
        const loadEncuentros = async () => {
            ...
            try {
                const response = await api.get('/public/encuentros');
                setEncuentros(response.data || []);
                if (response.data?.length > 0) {
                    setSelectedEncuentroId(String(response.data[0].id));
                }
            } catch (requestError) {
                console.error('Error loading public encuentros:', requestError);
                setError('No se pudieron cargar los encuentros disponibles.');
            } finally {
    ...
    const selectedEncuentro = useMemo(
        () => encuentros.find((encuentro) => String(encuentro.id) === String(selectedEncuentroId)),
        [encuentros, selectedEncuentroId]
    );
    ...
    const handleSubmit = async (event) => {
        ...
        if (!selectedEncuentroId) {
            setError('Selecciona un encuentro.');
            return;
        }
        ...
        try {
            ...
            toast.error(apiError || 'No se pudo enviar la inscripción.');
        } finally {
            setSubmitting(false);
        }
    };
                    ...
                    <section className="ln-card p-6 lg:p-8">
                        ...
                        {loadingEncuentros ? (
                            <div className="flex flex-col items-center justify-center min-h-[260px] text-[var(--ln-text-secondary)] gap-3">
                                ...
                                <span className="text-sm weight-510 text-[var(--ln-text-tertiary)]">Cargando encuentros...</span>
                            </div>
                        ) : encuentros.length === 0 ? (
                            <div className="flex flex-col items-center justify-center min-h-[260px] bg-[var(--ln-bg-panel)] rounded-2xl border border-dashed border-[var(--ln-border-standard)] p-8 text-center">
                                ...
                                <h3 className="text-base weight-590 text-[var(--ln-text-primary)] tracking-tight mb-1">Sin encuentros</h3>
                                <p className="text-sm text-[var(--ln-text-tertiary)] max-w-[280px]">No hay encuentros disponibles por el momento.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {encuentros.map((encuentro) => {
                                    const isSelected = String(encuentro.id) === String(selectedEncuentroId);

                                    return (
                                        <button
                                            key={encuentro.id}
                                            type="button"
                                            onClick={() => setSelectedEncuentroId(String(encuentro.id))}
                                        ...
                                        >
                                            ...
                                            <div className="mt-4 flex flex-col gap-2 text-sm text-[var(--ln-text-secondary)]">
                                                ...
                                                <span>
                                                    Donación: {formatCurrency(encuentro.cost)}
                                                </span>
                                                <span>
                                                    Inscritos: {encuentro.registeredCount || 0}
                                                </span>
                                            </div>
                                        </button>
                                    );
                    ...
                    <section className="ln-card p-6 lg:p-8">
                        <div className="mb-6">
                            ...
                            <h2 className="text-2xl weight-590 text-[var(--ln-text-primary)] tracking-[-0.3px]">
                                {selectedEncuentro ? selectedEncuentro.name : 'Encuentro'}
                            </h2>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {selectedEncuentro && ['HOMBRES', 'MUJERES'].includes(selectedEncuentro.type) && (
                                <div>
                                    <label className="block text-[11px] weight-590 uppercase tracking-widest mb-2 text-[var(--ln-text-tertiary)] ml-1">
                                        Sexo *
                                    </label>
                            ...
                            <div className="bg-white/[0.02] p-4 sm:p-5 rounded-xl border border-[var(--ln-border-standard)] space-y-3">
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    />
                                    <span className="text-[12px] sm:text-sm text-[var(--ln-text-secondary)] group-hover:text-[var(--ln-text-primary)] transition-colors leading-relaxed">Incluye libro U. de la V.</span>
                                </label>
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="mt-1 w-5 h-5 rounded border-[var(--ln-border-standard)] bg-[var(--ln-input-bg)] text-[var(--ln-brand-indigo)] focus:ring-[var(--ln-brand-indigo)] transition-colors"
                                    />
                                    <span className="text-[12px] sm:text-sm text-[var(--ln-text-secondary)] group-hover:text-[var(--ln-text-primary)] transition-colors leading-relaxed">Incluye otros gastos</span>
                                </label>
                            </div>

                            <button
                                type="submit"
                                disabled={submitting || loadingEncuentros || !selectedEncuentro}
...
export default PublicEncuentrosRegistration;
